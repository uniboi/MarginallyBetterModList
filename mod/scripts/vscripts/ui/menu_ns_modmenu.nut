...
void function InitModList()
{
	...
	Hud_GetChild( GetMenu("ModListMenu"), "SwtBtnShowFilter")
...
void function OnModMenuClosed()
{
	...
	if ( current.len() != file.enabledMods.len() || reload )
		ReloadMods()
}
...
void function OnModButtonFocused( var button )
{
	...
	var bgLabel = Hud_GetChild( file.menu, "StatusBackground" )
	bool enabled = NSIsModEnabled( modName )
	if ( enabled )
		Hud_SetColor( bgLabel, 0, 255, 0, 255 )
	else
		Hud_SetColor( bgLabel, 255, 255, 255, 255 )
	Hud_SetVisible( bgLabel, true )
